...
from .util.helper import demark
...
class Pose(Thread):
    def __init__(self, stream, corners, output_sz=(100, 130), id="right", demark=True):
    ...
    def img2grad(self, frame0, frame, bias=1.0):

        # blur = cv2.GaussianBlur(frame,(1,1),0)
        blur = frame
        # diff = blur * 1.0 - frame0
        diff = frame * 1.0 - cv2.GaussianBlur(frame0, (11, 11), 0)
        diff = diff * bias
    ...
    def img2depth(self, frame0, frame, bias=1.0):
        dx, dy = self.img2grad(frame0, frame)

        diff = (frame * 1.0 - frame0) / 255.0
    ...
    def get_pose(self):
        ...
        while self.running:
            ...
            if len(X) > 1:
                ...
                self.mv = np.mean(pts, 0)
                self.mv_relative = self.mv - [
                    frame0.shape[1] / 2,
                    frame0.shape[0] * 2 / 3,
                ]
                self.vr = self.mv - [frame0.shape[1], frame0.shape[0] * 2 / 4]
                self.theta_to_middle_right = asin(
                    self.vr[1] / np.sum(self.vr ** 2) ** 0.5
                )
                # print("theta_to_middle_right", self.theta_to_middle_right/pi*180, )

                if v_max[0] == 0:
                    cable_out = [0.0, self.mv[1]]
                else:
                    cable_out = [
                        0.0,
                        self.mv[1] - (self.mv[0]) / v_max[0, 0] * v_max[1, 0],
                    ]

                cable_out = np.array(cable_out)

                v_out = (
                    np.array([frame0.shape[1] / 2, frame0.shape[0] * 2 / 3]) - cable_out
                )
                v_out[1] *= -1
                ...
                cv2.ellipse(
                    diff,
                    (int(m[0]), int(m[1])),
                    (int(np.sum(v_max ** 2) ** 0.5), int(np.sum(v_min ** 2) ** 0.5)),
                    theta,
                    0,
                    360,
                    (255, 255, 255),
